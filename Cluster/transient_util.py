...
class transient_fem:
    ...
    def solver(self,theta_prev_time=None, theta_prev2_time=None, theta_prev_nr=None,\
        ...
        items = [(nodes, elei, source, theta_prev_time, theta_prev2_time, theta_prev_nr) 
                 for elei in ele]
        ...
        st = time.time()
        (M_row, M_col, M_data, K_row, K_col, K_data, G_row, G_col, 
         G_data, dMT_row, dMT_col, dMT_data, dKT_row, dKT_col, 
         dKT_data, dGT_row, dGT_col, dGT_data, F_row, F_data, BT_row, 
         BT_data, areas) = list(zip(*results))

        mega = [M_row, M_col, M_data, K_row, K_col, K_data, G_row, G_col, G_data,
                dMT_row, dMT_col, dMT_data, dKT_row, dKT_col, dKT_data, dGT_row, dGT_col, 
                dGT_data, F_row, F_data, BT_row, BT_data]
        ...
        if verbose == True:
            ...
            print(f"Spot radius is 0.75 mm")
...
def nr_iterative_rect(nodecoords, ele_con, source, theta_init, dt=1, t_final=1, 
                      type="transient", mode="non_linear"):
    ...
    for t in times:
        ...
        while(e>tolerance):
            iter +=1
            _,theta_cur_nr = transient_fem(nodecoords,ele_con,source).\
        ...
        print(f" Max temperature at {t}s: {np.max(theta_cur_nr)-273} degree celcius")
        theta_prev2_time = theta_prev_time.copy()
        theta_prev_time = theta_prev_nr.copy()
        source[0,0] = source[0,0] - laser_speed*dt  #moving left with 10 mm/s
        counter +=1
