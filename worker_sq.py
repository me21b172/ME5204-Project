...
def dK_dT(T):
    return (0.004*2*(T-50-273))*1e-3
   
def picard_square_helper(args):
...
def nr_square_helper(args):
    nodes,ele,theta_prev_time,theta_prev_pic,mode,scheme = args
    gp = 3
    
    K_row,K_col,K_data = [],[],[]
    M_row,M_col,M_data = [],[],[]
    dK_row,dK_col,dK_data = [],[],[]
    F_row,F_data = [],[]
    ...
    for k,ipk in enumerate(ips):
        ...
        if mode == "non_linear":
            kappa = N@k_T(theta_prev_pic[np.ix_(econ,[0])])
            dkappa = N@dK_dT(theta_prev_pic[np.ix_(econ,[0])])

        K_loc += kappa*a
        M_loc += rho*c*b
        X  =np.matmul(N,boundary)
        f_loc += N*Q(X)*np.linalg.det(Jac)*weights[k]
        dK_loc += kappa*a + dkappa*(a@theta_prev_pic[np.ix_(econ,[0])])@N
        area += np.linalg.det(Jac)*weights[k]    
    for i in range(nnode):
        if (f_loc.T[i,0]):
            F_row.append(econ[i])
        ...
        for j in range(nnode):
            ...
            if M_loc[i][j]!=0:
                ...
                M_data.append(M_loc[i][j])
            if dK_loc[i][j]!=0:
                dK_row.append(econ[i])
                dK_col.append(econ[j])
                dK_data.append(dK_loc[i][j])
                

                
    return K_row, K_col, K_data, M_row, M_col, M_data, dK_row,dK_col,dK_data,F_row,F_data,area
